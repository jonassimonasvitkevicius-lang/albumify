...
// Caches expensive Last.fm+Spotify responses so repeated page loads
// don't hammer the APIs. Popular/genre pages cached for 10 min,
// similar albums cached for 30 min (they don't change often).
...
app.get("/api/similar/:albumId", async (req, res) => {
  const cacheKey = `similar:${req.params.albumId}`;
  ...
  try {
    ...
    cacheSet(cacheKey, candidates.slice(0, 20).map(c => formatAlbum(c.album)), THIRTY_MIN);
  } catch (err) {
...
app.get("/api/genre-albums", async (req, res) => {
  ...
  try {
    ...
    cacheSet(cacheKey, results, TEN_MIN);
    res.json(results);
  } catch (err) {
...
app.get("/api/album-of-the-day", async (req, res) => {
  ...
  try {
    ...
    const result = { ...formatAlbum(found), date: dateStr };
    cacheSet(cacheKey, result, ONE_DAY);
    res.json(result);
  } catch (err) {
...
app.get("/api/new-releases", async (req, res) => {
  ...
  try {
    ...
    const result = unique.slice(0, 20).map(formatAlbum);
    cacheSet("popular", result, TEN_MIN);
    res.json(result);
  } catch (err) {
